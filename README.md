...
界面上有一行 **`ADB 服务: ● 运行中  [停止]  ☑ 退出时停止 ADB 服务`**。

| 控件 | 作用 |
|---|---|
| 状态文字 | 实时显示 adb 服务端在不在跑（每 2.5 秒查一次，毫秒级端口探测） |
| `[启动]` / `[停止]` | 手动启停 adb 服务端 |
| ☑ 退出时停止 | **默认勾选** —— 关窗时自动 `adb kill-server` |

**为什么要给这个开关** —— adb 服务端（监听 `5037`）是**多个工具共用**的常驻进程（Android Studio、scrcpy 用的是同一个）。本程序不会在运行期间擅自杀它，只在你显式点「停止」或勾选退出时停止。
...
③ adb kill-server         按勾选停掉服务端（必须在 ② 之后）
...
| 版本 | 变化 |
|---|---|
| **1.1.1** | **新增 ADB 服务开关**（可手动启停 + 退出时自动停止）；**修复关窗后 `adb.exe` 变成孤儿进程**；**修复拖拽窗口卡顿**（每帧 146ms → 54ms）；**修复窗口缩放后界面元素被隐藏** —— 改为响应式布局：横排控件自动折行、可滚动画布兜底、窗口尺寸按屏幕实测决定 |
